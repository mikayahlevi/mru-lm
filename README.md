...
$W_{in}$ is a $h \times d_c \times d_h$ tensor, which has the result of essentially matrix-multiplying each head of $\text{reshape}(x, h, d_o, d_c)$ by a unique weight matrix.
...
Therefore, $W_{out}$ is a $h \times d_h \times d_c$ tensor, which also has the effect of matrix-multiplying each head by a unique matrix.
